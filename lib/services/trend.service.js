...
`;
    const [result] = await connection.execute(statement, [pageSize, (currentPage - 1) * pageSize <= 0 ? 0 : (currentPage - 1) * pageSize]);
    return result;
  }
  ...
  async postTrendToUser_Trend(user_id, trend_id) {
    ...
    const [result] = await connection.execute(statement, [user_id, trend_id]);
    return result;
  }
